                ...
                <div class="panel panel-default">
                    <div class="panel-heading">
                        ...
                        @can('create-inbound-shipments')
                            <a href="{{ route('manifest.create') }}" class="btn btn-primary btn-xs pull-right">
                            ...
                            @foreach($manifests as $manifest)
                                <tr>
                                    @if(Auth::user()->can('view-inbound-shipments'))
                                        <td><a href="{{ route('manifest.show', $manifest->id) }}">MAN-{{ str_pad($manifest->id, 5, '0', STR_PAD_LEFT) }}</a></td>
                                    @else
                                    ...
                                    {{--<td class="text-right">${{ number_format($manifest->cbv_rate, 0) }}/KG</td>--}}
                                    {{--<td class="text-right">{{ number_format($manifest->consignment_weight, 0) }} KGs</td>--}}
                                    <td>
