            ...
            <div class="col-sm-12">
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        ...
                        <form action="/clearance" method="post" role="form" target="_blank" @submit="refreshPage">
                            <input type="hidden" name="_token" :value="$root.csrf">
                            <input type="hidden" name="waybills" :value="waybillAll">
                            ...
                            <div class="form-group">
                                <button type="submit" class="btn btn-success" :disabled="waybills.length < 1">Process Release Order</button>
                            </div>
                        </form>
...
<script>
    export default {
        ...
        methods: {
            ...
            updateWaybills() {
                this.$root.isLoading = true;
                axios.get('/manifest?waybills=true&manifest_id=' + this.manifest_id).then(response => {
                    this.waybills = response.data.waybills;
                    this.waybillAll = JSON.stringify(response.data.waybills);
                    this.$root.isLoading = false;
                }).catch(() => this.$root.isLoading = false);
            },
